...
df_conf1 <- df_conf %>% tidyr::pivot_longer(
...
df_death1 <- df_death %>% tidyr::pivot_longer(
  cols = dplyr::starts_with("X"),
  names_to = "date_temp",
  values_to = "cases_temp"
)
...
df_rec1 <- df_rec %>% tidyr::pivot_longer(
  cols = dplyr::starts_with("X"),
  names_to = "date_temp",
  values_to = "cases_temp"
)
...
writexl::write_xlsx(x = coronavirus, path = "coronavirus_dataset.xlsx", col_names = TRUE)
