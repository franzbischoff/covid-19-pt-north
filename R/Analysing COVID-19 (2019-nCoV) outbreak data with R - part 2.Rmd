...
chinese_wikipedia_data_url <- paste("https://en.wikipedia.org/w/index.php?title=",
  "Template:2019–20_coronavirus_pandemic_data/",
  "China_medical_cases_by_province&oldid=943292438",
  sep = ""
) # 944340135
...
excl_hubei <- chinese_provinces_confirmed %>%
...
pred_decay_incidence <- tibble(
  dates = hubei_incidence_peak + 0:99,
...
japanese_prefectures_confirmed %>%
  ...
  geom_bar(stat = "identity") +
  labs(
    y = "Incident cases", fill = "Recent travel to China",
...
j <- japanese_prefectures_confirmed %>%
  ...
  facet_wrap(Prefecture ~ ., scales = "free_y", ncol = 2) +
  labs(
    y = "Daily incremental incidence",
...
prefecture_plot_lambda <- function(res) {
  plot(res, "lambdas",
    scale = length(res$local_case_dates) + 1,
    bty = "n"
  )
  title(sub = paste(
