...
control <- control.icm(type = "SIR", nsteps = 100, nsims = 1, verbose.int = 1, verbose = TRUE)
```
...
```{r, echo=TRUE, eval=TRUE}
init <- init.icm(s.num = 10260890, i.num = 3, r.num = 0)
```
...
sims_incidence_rates <- sims_incidence_rates %>%
  mutate(
    act_rate_facet_label = paste(act_rate, "exposures per day"),
...
sims_incidence_rates %>%
  ...
  labs(
    x = "Days since start of epidemic", y = "New cases per day",
...
# set up an empty data frame to which to append
# results from each simulation
big_sims_incidence_rates <- tibble(
  time = integer(0),
  si.flow = numeric(0),
  i.num = numeric(0),
  act_rate = numeric(0),
