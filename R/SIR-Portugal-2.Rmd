...
base_path <- which(c(file.exists("Covid-19.Rproj"), file.exists("../Covid-19.Rproj"), file.exists("../../Covid-19.Rproj")))
paths <- c("./", "../", "../../")
base_path <- paths[base_path]

for (source_file in source_files) {
  source(paste0(base_path, "R/ext_exp/", source_file), encoding = "UTF-8")
  # source(paste0("./ext/", source_file))
}

source(paste0(base_path, "R/plot_sir.R"), encoding = "UTF-8")
...
report_rate_list[["0409"]] <- rep(0.25, 20)

report_rate_list[["0419"]] <- c(
...
gr_etario <- cases_dataset %>%
  filter(data_relatorio == version_date) %>%
  select(gr_etario_0_9:gr_etario_80_89)
...
pt_population_norte <- 3574394 / 100 # est 2019 ****** 3690681
...
simulation_m1 <- simulate(type = "SEIQHRF",
  nsteps = steps,
  nsims = 8,
  ncores = 8,
  ...
  #################
  #################
  disch.rand = TRUE, # If TRUE, binomial, if FALSE, random samplse.
...
plot_times <- get_times(simulation_1)
...
plot_ly_sir(simulation_1, c("e.num", "i.num", "q.num", "h.num", "f.num", "r.num"), events = events) %>% add_annotations(
  x = 1,
  y = 1,
  xref = "paper",
  yref = "paper",
...
plot_ly_sir(simulation_m1, c("e.num", "i.num", "q.num", "h.num", "f.num", "r.num"), events = events) %>% add_annotations(
  x = 1,
  y = 1,
  xref = "paper",
  yref = "paper",
...
plot_ly_sir(simulation_1, c("f.num", "h.num", "i.num", "q.num", "e.num", "r.num"), stacked = TRUE, grouped = TRUE, real_data = geo_norte, events = events, log = TRUE) %>% add_annotations(
  x = 1,
  y = 1,
  xref = "paper",
  yref = "paper",
...
plot_ly_sir(simulation_m1, c("f.num", "h.num", "i.num", "q.num", "e.num", "r.num"), stacked = TRUE, grouped = TRUE, real_data = geo_norte, events = events, log = TRUE) %>% add_annotations(
  x = 1,
  y = 1,
  xref = "paper",
  yref = "paper",
...
fig <- plot_ly(x = ~ res$R_grid, y = ~ res$R_like) %>%
  add_lines() %>%
  add_annotations(
    x = 1,
    y = 1,
